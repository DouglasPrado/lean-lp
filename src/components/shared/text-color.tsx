...
export function TextColor({ words, className }: TextColorProps) {
  return (
    <>
      <TextColorStyles />
      <span
        className={cn("inline-flex flex-wrap items-center justify-center select-none", className)}
      >
        {words.map((word, i) => (
          <span key={i} className="relative px-1 sm:px-2">
            {/* Solid text — visible when gradient is hidden */}
            <span style={{ animation: `tc-bg-${i + 1} 8s infinite` }}>{word}</span>

            {/* Gradient text — cycles in with color */}
            <span
              className="absolute inset-0 bg-clip-text px-1 text-transparent sm:px-2"
              style={{
